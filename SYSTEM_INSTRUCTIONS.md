...
5. **AI Bridge Protocol:** Be aware of the directories `internal_ignore/inbox_scribe/` and `internal_ignore/inbox_gemini_cli/`. Use these to pass complex context, schemas, or large tracebacks back and forth with Gemini Scribe (the architectural planner in Obsidian). Create new files with a numeric prefix and short description (e.g., `01_error_traceback.md`) when sending a message.
