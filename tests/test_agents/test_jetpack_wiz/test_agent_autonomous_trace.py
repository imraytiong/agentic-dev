...
async def main():
    print("🚀 Starting AUTONOMOUS API TRACE test (REAL GEMINI LLM)...\n")
    ...
    context = AgentContext(user_id="trace_user_7", session_id="trace_session_7", tenant_id="local_dev")

    # Step 1: Pre-set the focus to compose/ui
    state_key = f"jetpack_wiz_state_{context.session_id}"
    from src.agents.jetpack_wiz.models import JetpackWizState, ActiveFocus
    state = JetpackWizState(has_indexed=True, active_focus=ActiveFocus(module_path="compose/ui"))
    ...
    # We will simulate multiple turns to capture the triage loop
