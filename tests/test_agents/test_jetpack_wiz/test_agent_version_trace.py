...
async def main():
    print("\n==========================================================================")
    print("🚀  JETPACKWIZ AGENT - VERSION HISTORY TRACE TEST 🚀")
    print("==========================================================================\n")
    ...
    context = AgentContext(user_id="trace_user_version", session_id="trace_session_version", tenant_id="local_dev")

    # Step 1: Pre-set the focus to room/room-runtime
    state_key = f"jetpack_wiz_state_{context.session_id}"
    from src.agents.jetpack_wiz.models import JetpackWizState, ActiveFocus
    state = JetpackWizState(has_indexed=True, active_focus=ActiveFocus(module_path="room/room-runtime"))
