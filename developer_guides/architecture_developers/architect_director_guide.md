...
2. Type: `load adk-core-builder` (view the skill instructions here: **[adk-core-builder](../../skills/adk-core-builder/SKILL.md)**)
3. The CLI will initialize and ask for the spec. Copy the contents of **[Universal Core Architecture Spec](../../src/universal_core/universal_core_architecture_spec.md)** (from the `src/universal_core` folder) and paste it into the prompt.
...
*   **Layer 1: The Contracts (`src/universal_core/interfaces.py`).** It must define the Abstract Base Classes (`BaseStateStore`, etc.) first.
*   **Layer 2: The Core Object (`src/universal_core/chassis.py` - Init).** It must write the `__init__` method, the `config.yaml` deep merge, and the `importlib` dynamic loading.
...
Once the `src/universal_core/` directory passes the Golden Sanity Check, your job as the primary coder is done. 

1. Zip the `src/universal_core/` directory.
2. Hand it to **Track A (Infrastructure Leads)**. Tell them: *"Here is the Universal Core. Do not modify it. Build the adapters in the `src/infrastructure/` folder to satisfy `src/universal_core/interfaces.py`."*
